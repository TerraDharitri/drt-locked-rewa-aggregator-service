  ...
  private mapLogs(nonce: string, res: any, index: number, historyLog: AssetHistoryLog[]) {
    ...
    switch (eventCategory) {
      ...
      case NftEventTypeEnum.NumbatNftsSwapAuctionEventEnum: {
        this.addHistoryLog(historyLog, this.assetsHistoryNumbatNftsSwapEventsService.mapNumbatNftsSwapEventLog(eventType, mainEvent));
        break;
      }
    }
  }
    ...
    for (let i = 0; i < relatedEvents?.length; i++) {
      ...
      for (let j = 0; j < events.length; j++) {
        ...
        if (Object.values(KroganSwapAuctionEventEnum).includes(eventIdentifier)) {
          return [NftEventTypeEnum.NumbatNftsSwapAuctionEventEnum, eventIdentifier, relatedEvents[i]];
        }
      }
    }
