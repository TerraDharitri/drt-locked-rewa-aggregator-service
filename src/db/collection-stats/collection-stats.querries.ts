...
export function getCollectionStats(identifier: string, marketplaceKey: string = undefined, paymentToken: string = drtConfig.rewa) {
  return `
  WITH
...
export function getCollectionFloorPrice(identifier: string, marketplaceKey: string = undefined, paymentToken: string = drtConfig.rewa) {
  return `
