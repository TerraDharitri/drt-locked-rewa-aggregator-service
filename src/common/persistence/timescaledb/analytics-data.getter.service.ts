...
export class AnalyticsDataGetterService {
  ...
  async getVolumeDataWithMarketplaces({ time, series, metric, start }: AnalyticsArgs): Promise<AnalyticsAggregateValue[]> {
    ...
    const query = await this.sumMarketplaceDaily
      ...
      .addSelect(
        'sum(sum) as value, sum(xoxno) as xoxno, sum(frameit) as frameit, sum(numbatapes) as numbatapes, sum(deadrare) as deadrare, sum(hoghomies) as hoghomies, sum(numbatnftswap) as numbatnftswap, sum(aquaverse) as aquaverse, sum(holoride) as holoride, sum(eneftor) as eneftor,sum(ici) as ici  ',
      )
